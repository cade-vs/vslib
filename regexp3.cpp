...
int regtry( regexp *prog, const char *string); /* 0 failure, 1 success */
...
int regmatch( const char* pattern, const char* string )
{
  ...
  if (!re) return 0;
  int res = regexec( re, (char*)string );
  free( re );
  return res;
}
...
int regmatchpos( const char* pattern, const char* string )
{
  ...
  if (!re) return -1;
  int res;
  if ( regexec( re, (char*)string ) )
    {
    res = re->startp[0] - pattern;
    }
  else
    {
    res = -1;
			...
			while (*regparse != '\0' && *regparse != ']') {
				if (*regparse == '-') {
					...
					if (*regparse == ']' || *regparse == '\0')
						...
						class_end = UCHARAT(regparse);
						if (class_ > class_end+1)
							FAIL("invalid [] range");
						for (; class_ <= class_end; class_++)
							regc(class_);
						regparse++;
					}
				} else
